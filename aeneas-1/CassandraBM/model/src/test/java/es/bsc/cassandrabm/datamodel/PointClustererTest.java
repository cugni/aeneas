...
//        }
    ...
    public PointType randomP(PointType from, PointType to) {
        PointType p = new PointType();
        p.setX(rangeRand(from.getX(), to.getX()));
        p.setY(rangeRand(from.getY(), to.getY()));
        p.setZ(rangeRand(from.getZ(), to.getZ()));
        return p;
