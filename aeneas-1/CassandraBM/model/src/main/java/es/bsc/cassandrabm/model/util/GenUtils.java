...
public class GenUtils {
    ...
    static {
        try {
            jc = JAXBContext.newInstance("es.bsc.cassandrabm.model.gen");
        } catch (Exception e) {
...
//            checkArgument(cassandraModelFile.exists(), "cassandraModel not found. {0}", cassandraModelFile.getAbsolutePath());
    ...
    public static ModelType getQueryModel() {
        try {
//            checkArgument(queryModel.exists(), "queryModel not found. {0}", queryModel.getAbsolutePath());
            Unmarshaller um = jc.createUnmarshaller();
            SchemaFactory sf = SchemaFactory.newInstance(
                    javax.xml.XMLConstants.W3C_XML_SCHEMA_NS_URI);
            um.setSchema(sf.newSchema(queryModelXSD));
            @SuppressWarnings("unchecked")
            JAXBElement<ModelType> jk =
                    (JAXBElement<ModelType>) um.unmarshal(queryModel);
            return jk.getValue();
        } catch (Exception ex) {
            Logger.getLogger(GenUtils.class.getName()).log(Level.SEVERE, null, ex);
            throw new IllegalStateException(ex);
    ...
    static Model getQueryImplementation(String qiname) {
        return getQueryImplementation(getQueryImplementationFile(qiname));
    }
    
    public static Model getQueryImplementation(URL queryIml) {
        try {
//            checkArgument(queryIml.exists(), "queryModel not found. {0}",
//                    queryModel.getAbsolutePath());
            Unmarshaller um = jc.createUnmarshaller();
            SchemaFactory sf = SchemaFactory.newInstance(
                    javax.xml.XMLConstants.W3C_XML_SCHEMA_NS_URI);
            um.setSchema(sf.newSchema(queryImlXSD));
            return (Model) um.unmarshal(queryIml);
